    ...
    class ServerMain
    {
        ...
        public static void Main(string[] args) {
            ...
            if (!server.Setup(new ServerConfig()
            {
                ...
                DisableSessionSnapshot = true,// ВАЖНО если не отключить(true- это отключено), то снимок сеансов делается раз в sessionSnapshotInterval (5сек по умолчанию), и метод GetAllSession() выдает не актуальные данные пока не сделется новый снимок
                Mode = SocketMode.Tcp,
                TextEncoding = "UTF-8",
                Ip = "any"
                },null,null,
